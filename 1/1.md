...
Magnitude means size, or *scale*. Scalars are quantities that ONLY have scale, most commonly stored as a `float` or `int`. Note that `float` and `int` are signed types, so if the result of a calculation can be either positive or negative, they CAN encode direction. In that case they wouldn't represent a scalar quantity but a 1-dimensional vector.
